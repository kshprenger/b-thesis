...
Преимуществами использования программной транзакционной памяти является то, что она поддерживается на любых платформах, которые поддерживают атоманые операции над одной ячейкой памяти. Она также наследует все преимущества неблокирующей синхронизации.

Существенным недостатком этого вида синхронизации является то, что транзакционная память может оперировать только над ячейками одного типа, определяемым языком программирования. Из-за чего возникает необходимость в небезопасной конвертации типов.
