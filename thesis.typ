...
В данном тесте прирост относительно небольшой, так как при малом числе конкурирующих потоков, использование блокирующей синхронизации эффективно.
...
Можно дополнительно заметить, что прирост в данном случае больше чем, при сравнении с режимом 1/4. Связано это с тем, что потоки-писатели создают большую нагрузку на канал за счёт оперирования с разделяемым состоянием через Mutex, вместо параллельного RwLock для потоков-читателей. Это же замечание справедливо для тестов режимов 1/32 и 32/1
