...
На каждый тест создаётся N задач для потоков-отправителей и K задач для потоков получателей. Длина буффера для канала в каждом тесте составляет 1000 элементов. Каждый из потоков отправителей отправляет по 10 сообщений. Отдельный тествовый случай с определённым числом задач для потоков-отправителей и получателей исполняется по 100 раз.
